...
class FlowController extends Controller
{
     ...
     */
    public function removeCorrelation(Request $request, $flow_id)
    {
        $act_key = $request->get('act_key');
        //先找到该流程处于活动的第几阶段 arr_key 表示
        $act_m = new ActDesign();
        $need = ['flow_structure', 'current_flow', 'activity_secret'];
        if (!$act = $act_m->getActInfo(['activity_key' => $act_key], $need))
            return response()->json(['status' => 0, 'message' => '解除关联失败'], 400);
        //第几阶段,从零开始计数
        $flow_list = explode(',', $act['flow_structure']);
        $arr_key = array_search($flow_id, $flow_list); //array_search没有找到返回false
        if($arr_key == false){
            return response()->json(['status' => 0, 'message' => '该流程不属于act_key所对应的活动'], 403);
        }

        //如果该流程被关联
        $flow_m = new FlowInfo();
        if (!$flow_info = $flow_m->getFlowInfo(['flow_id' => $flow_id], '*'))
            return response()->json(['status' => 0, 'message' => '解除失败'], 403);
        if (empty($flow_info['correlation']))
            return response()->json([
                'status' => 0,
                'message' => '该流程没有被设置关联，无需解除'
            ], 400);

        $secret_list = explode(',', $flow_info['correlation']);
        $secret_key = array_search($act['activity_secret'], $secret_list);
        ...
        //删除原流程中的需要解除关联的secret
        array_splice($secret_list, $secret_key, 1); //去除被选流程secret
        if (empty($secret_list))
            $new_correlation = '';
        else {
            $new_correlation = reset($secret_list);
            for ($i = 1; $i < count($secret_list); $i++)
                //重构correlation
                $new_correlation = $new_correlation . ',' . next($secret_list);
        }
        $flow_update['correlation'] = $new_correlation;

        //备份一个流程替代原来的流程
        $useless = ['flow_id', 'correlation', 'sms_temp_id', 'SmsVariables', 'sms_temp', 'created_at', 'updated_at'];
        $flow_info = json_decode($flow_info, true);
        $new_flow_info = unset_idle($flow_info, $useless);
        $new_flow_info['activity_key'] = $act_key;

        //手动开始事务
        DB::beginTransaction();
        if (!$flow_m->updateFlowInfo(['flow_id' => $flow_id], $flow_update))
            return response()->json([
                'status' => 0,
                'message' => '解除失败'
            ], 400);
        if (!$new_flow = $flow_m->storeNewFlow($new_flow_info)) {
            DB::rollBack();
            return response()->json([
                'status' => 0,
                'message' => '解除失败'
        ...
        $flow_list[$arr_key] = $new_flow['flow_id'];
        $new_flow_structure = $flow_list[0];
        for ($i = 1; $i < count($flow_list); $i++) {
            //重新构造flow_structure
            $new_flow_structure = $new_flow_structure . ',' . $flow_list[$i];
        }
        $act_update['flow_structure'] = $new_flow_structure;
        if (!$act_m->updateActInfo(['activity_key' => $act_key], $act_update)) {
            DB::rollBack();
            return response()->json([
                'status' => 0,
                'message' => '解除失败'
