...
class Base
{
    ...
    public function handle($request, Closure $next)
    {
        ...
        if (!$res = (new ActDesign())->getActInfo(['activity_id' => $act_key], ['author_id', 'activity_name', 'current_flow']))
            return response()->json(['status' => 0, 'message' => '无此活动'], 404);
        else {
            $act_name = $res['activity_name'];
            $current_flow = $res['current_flow'];
            if ($res['author_id'] != $author_id)
                return response()->json(['status' => 0, 'message' => '非法请求'], 403);
        }

        $request->attributes->add(compact('current_flow'));
